...
function App() {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <div className="relative z-10 container mx-auto px-4 pt-0 pb-4 min-h-screen flex flex-col items-center">
        
       {/* Header */}
<div className="text-center mb-1">
  <div className="flex items-center justify-center mb-1">
    <div className="relative inline-block">
      {/* Tight glow behind logo */}
      <div 
         className="absolute inset-0 m-auto w-[75%] h-[35%] bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 opacity-80 blur-[2px]"
  style={{
    animation: 'glowRunning 2s linear infinite',
      ...
      <img 
        ...
        className="relative h-auto md:h-80 w-99 block"
        style={{
          filter: 'drop-shadow(0 0 8px rgba(59, 130, 246, 0.9)) drop-shadow(0 0 15px rgba(236, 72, 153, 0.7))',
        }}
      />
    </div>
  </div>
  <div className="flex items-center justify-center gap-3">
    <div className="h-px w-20 bg-gradient-to-r from-transparent via-blue-400 to-transparent" />
    <p className="text-cyan-300 text-sm md:text-base flex items-center justify-center gap-1.5 animate-pulse">
    ...
    <div className="h-px w-20 bg-gradient-to-r from-transparent via-blue-400 to-transparent" />
        ...
        <div className="w-full max-w-sm mb-3">
          <Label className="text-slate-300 text-sm mb-1 block">Select Currency</Label>
          <div className="relative">
            <button 
              className="w-full flex items-center justify-between bg-slate-900/60 border border-slate-700/50 rounded-lg px-4 py-2 text-white hover:bg-slate-900/80 transition-colors backdrop-blur-sm"
        ...
        {/* Live Price Card */}
        <div className="w-full max-w-sm mb-3 relative group">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <button 
                    ...
                    className={`px-2 py-1 rounded-md text-xs font-medium transition-colors ${!isLive ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-slate-500 hover:text-slate-400'}`}
                  >
                    Cached
                  </button>
                  <button 
                    onClick={() => setIsLive(true)}
                    className={`px-2 py-1 rounded-md text-xs font-medium transition-colors flex items-center gap-1 ${isLive ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'text-slate-500 hover:text-slate-400'}`}
                  >
        ...
        <div className="w-full max-w-sm mb-3 relative group">
          ...
          <Card className="relative bg-slate-950/80 border-0 backdrop-blur-xl">
            <CardContent className="pt-4 space-y-3">
              {/* My Price Input */}
              <div>
                <Label className="text-slate-300 flex items-center gap-2 mb-1 text-sm">
                ...
                <Input
                  ...
                  className="bg-slate-900/60 border-slate-700/50 text-white placeholder:text-slate-600 focus:border-cyan-500/50 h-9"
                />
